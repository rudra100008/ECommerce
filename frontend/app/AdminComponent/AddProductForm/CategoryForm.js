...
import {
  addCategory,
  validateCategory,
} from "../../services/adminServices/ProductCategoryServices";
...
export default function CategoryForm({ setState, formData, updateFormData }) {
  ...
  const handleNext = async (e) => {
    e.preventDefault();
    try {
      await validateCategory(formData.category);
      setState("product");
    } catch (error) {
  ...
  const handleCategoryItemClick = (category) => {
    ...
    const currentCategoryName =
      typeof formData.category === "string" ? "" : formData.category.name || "";
          ...
          {categories.map((category, index) => (
            <div
              ...
              className={`${style.categoryItem} ${
                formData.category.name === category.name ? style.selected : ""
              }`}
