...
import {
  ...
  removeAddressById,
  updateAddress,
} from "../services/clientServices/AddressService";
...
export default function Profile() {
  ...
  const fetchAllAddresses = async () => {
    try {
      const addresses = await fetchAllAddressesOfUser(
    ...
    try {
      ...
      setUserData((prev) => ({
        ...
        addresses: prev.addresses.filter(
          (prevAddress) => prevAddress.addressId !== address.addressId
        ),
      }));
    } catch (err) {
                ...
                <div className={style.addressTitle}>
                  ...
                  {address.addressId !== editingAddressId ? (
                    ...
                    <>
                      ...
                      <button
                        className={style.cancelAddressButton}
          ...
          <div className={style.settingContainer}>
            {userData.hasCustomImage && (
              <div className={style.revertPicButtonSection}>
                <button
                  className={style.revertPicButton}
