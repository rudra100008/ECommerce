...
export const fetchRandomProduct = async ({ pageNumber = 0, pageSize = 9 }) => {
  try {
    ...
    console.log(
      "Response in ProductService of fetchRandomProductByCategoryId():",
      res
    );
    return res;
  } catch (error) {
    console.log(
      "Error in ProductService of fetchRandomProductByCategoryId():",
      error.response?.data
    );
    throw error;
  }
...
export const findProductsByIds = async (productIds = []) => {
  ...
  try {
    const response = await api.post(
      `/api/product/fetchAllProductsByIds`,
      productIds
    );
    return response.data;
  } catch (err) {
