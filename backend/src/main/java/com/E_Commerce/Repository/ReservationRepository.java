...
public interface ReservationRepository extends JpaRepository<Reservation, Integer> {
    ...
    @Query("SELECT r FROM Reservation r WHERE r.user.userId =:userId " +
            ...
            "AND r.status = 'CONVERTED_TO_ORDER' " +
            "AND r.expiresAt > :now ")
    List<Reservation> findConvertedToOrderReservationByUserAndProducts(
            @Param("userId") Integer userId,
            @Param("productIds") List<Integer> productIds,
            @Param("now") LocalDateTime now
    );
    ...
    @Query(value = """
                            INSERT INTO reservations
                                (user_id, inventory_id, reserved_quantity, reserved_at, expires_at, status)
                            VALUES
                                (:userId, :inventoryId, :quantity, :now, :expires, 'ACTIVE')
